    ...
    public class MTLSourceChecker
    {
        ...
        public async Task<ChapterSourceCheckResult> GetUpdateAsync(string mostRecentChapterId)
        {
            ...
            foreach (var cNode in nodes)
            {
                ...
                var chapterNode = cNode.SelectSingleNode("//a[@class='chapter-link']");
                if (chapterNode == null)
                {
                ...
                if (chapterNumberString?.Length < 1 || !int.TryParse(chapterNumberString.Substring(1), out int chapterNumber))
                {
                ...
                string chapterTitle = "";
                if (chapterNode.ChildNodes?.Count < 3 || string.IsNullOrEmpty(chapterNode.ChildNodes[2].InnerText))
                {
                ...
                var metaDataNode = cNode.SelectSingleNode("//td[@class='text-right']");
